...
## 更新摘要
**变更内容**
- 更新了药品类型和单位选项的实现，从动态配置改为硬编码固定选项
- 修订了表单字段设计部分，反映了简化的选择器实现
- 更新了架构概览，体现了固定的预设选项设计模式
...
**更新** 系统现已简化为使用固定预设的药品类型和单位选项，不再支持用户自定义配置。表单中的类型选择和单位选择都改为硬编码的固定选项列表，提升了系统的稳定性和一致性。
...
- 药品名称：必填字段，用于标识药品
- 有效期：必填字段，关键的安全信息
- 类型：下拉选择，使用固定预设的三种类型选项

**更新** 类型选择器现在使用硬编码的固定选项列表：
- 内服（internal）
- 外用（external）  
- 急救（emergency）

这些选项直接在组件内部定义，不再从数据库或配置中动态加载，确保了选项的一致性和稳定性。

**扩展信息区域（可选）**
- 服用/使用说明：详细指导文本
- 剩余数量：库存管理
- 单位：下拉选择，使用固定预设的计量单位选项
- 生产厂商：供应商信息

**更新** 单位选择器同样使用硬编码的固定选项列表：
- 片
- 粒  
- 支
- 瓶
...
这些单位选项直接在组件内部定义，提供了常用且实用的药品计量单位选择。
...
**更新** 固定选项的变更不会影响表单功能，因为所有选项都是硬编码定义的，确保了稳定性和一致性。
...
**更新** 系统现已采用简化的固定预设选项设计，这种设计模式提供了以下优势：
- **稳定性**：硬编码选项确保了选项的一致性和可靠性
- **性能**：避免了动态加载选项的开销
- **维护性**：减少了配置管理的复杂性
- **用户体验**：固定的选项列表提供了清晰明确的选择
