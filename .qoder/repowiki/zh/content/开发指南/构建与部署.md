...
## 更新摘要
**变更内容**   
- 新增 GitHub Actions 自动发布流程，支持版本同步和多平台构建
- 添加 APK 动态重命名功能，支持版本号自动注入
- 更新构建系统配置以支持动态版本显示
- 完善 CI/CD 流程中的版本管理策略
...
VersionSync["版本同步<br/>自动更新版本号"]
APKRename["APK 重命名<br/>动态版本注入"]
end
Pkg --> Vite
Vite --> Tauri
React --> Vite
...
- [.github/workflows/release.yml:1-326](file://.github/workflows/release.yml#L1-L326)
...
## 核心组件
- 前端构建与开发服务器：Vite 配置启用 React 与 Tailwind 插件，开发服务器端口与热更新配置，忽略 src-tauri 目录监听。**新增** 动态版本显示功能，通过 `__APP_VERSION__` 宏注入当前版本号。
...
- CI/CD：GitHub Actions 在 macOS、Windows、Ubuntu 上分别构建 macOS、Windows、Android；**新增** 自动版本同步和 APK 重命名功能；最后汇总产物并创建 GitHub Release。
...
- [build.gradle.kts:1-29](file://src-tauri/gen/android/build.gradle.kts#L1-L29)
- [app.build.gradle.kts:1-72](file://src-tauri/gen/android/app/build.gradle.kts#L1-L72)
- [.github/workflows/release.yml:25-40](file://.github/workflows/release.yml#L25-L40)
- [.github/workflows/release.yml:264-276](file://.github/workflows/release.yml#L264-L276)

## 架构总览
下图展示了从开发到发布的整体流程，包括前端构建、Tauri 打包、Android 构建与 GitHub Actions 发布，**新增** 版本同步和 APK 重命名流程。
...
participant VersionSync as "版本同步"
participant APKRename as "APK 重命名"
...
Android-->>Dev : 生成 APK
Dev->>GH : 推送标签/触发工作流
GH->>VersionSync : 同步版本号到配置文件
VersionSync-->>GH : 更新后的配置文件
GH->>Tauri : 并行构建 macOS/Windows
GH->>Android : 构建 Android
GH->>APKRename : 重命名 APK 文件
APKRename-->>GH : Assetly_vX.X.X_aarch64.apk
...
- [.github/workflows/release.yml:25-40](file://.github/workflows/release.yml#L25-L40)
- [.github/workflows/release.yml:264-276](file://.github/workflows/release.yml#L264-L276)
- [.github/workflows/release.yml:17-326](file://.github/workflows/release.yml#L17-L326)
...
- **新增** 动态版本显示：通过 `define` 配置注入 `__APP_VERSION__` 宏，从 package.json 读取当前版本号，用于应用内版本展示。
...
**章节来源**
- [tauri.macos.conf.json:1-10](file://src-tauri/tauri.macos.conf.json#L1-L10)
- [bundle_dmg.sh:1-200](file://src-tauri/target/universal-apple-darwin/release/bundle/dmg/bundle_dmg.sh#L1-L200)
...
- **新增** 版本同步：每个平台构建前都会同步版本号到 `tauri.conf.json` 和 `package.json` 文件。
- 并行作业：
  ...
  - Android：安装 Node.js、pnpm、Java 17、Android SDK，安装多架构 Rust 目标，初始化并构建 APK（aarch64），**新增** APK 重命名功能。
...
- **新增** APK 重命名：自动将 APK 文件重命名为 `Assetly_{VERSION}_aarch64.apk` 格式，便于识别版本。

**章节来源**
- [.github/workflows/release.yml:176-284](file://.github/workflows/release.yml#L176-L284)
- [app/build.gradle.kts:58-60](file://src-tauri/gen/android/app/build.gradle.kts#L58-L60)

### 版本管理策略
- 语义化版本：前端与后端版本号保持一致，遵循主.次.修订格式。
- **新增** 自动版本同步：GitHub Actions 工作流会在构建前自动同步标签版本到配置文件。
...
GH["release.yml"] --> VersionSync["版本同步"]
VersionSync --> TauriConf
VersionSync --> Pkg
GH --> TauriCli
...
- **新增** 版本同步：自动版本同步操作会修改配置文件，建议在 CI 环境中合理使用缓存策略。
...
- **新增** CI 发布失败：核对标签格式（v*）与 GITHUB_TOKEN 权限；检查产物路径与上传匹配；验证版本同步是否成功。
- **新增** APK 重命名失败：检查版本号格式是否正确，确认 APK 文件路径是否存在。
...
## 结论
本指南基于仓库现有配置，系统梳理了 Assetly 的多平台构建与部署方案。通过 Vite + Tauri 的组合实现跨平台桌面应用，配合 GitHub Actions 实现自动化构建与发布；Android 通过 Gradle 与 Rust 插件完成原生库集成与 APK 生成。**最新更新** 包含了自动版本同步、APK 重命名和动态版本显示等新功能，显著提升了发布流程的自动化程度和用户体验。建议在后续迭代中完善签名与证书管理、增强发布说明与变更日志、建立回滚与监控机制，以提升发布质量与用户体验。
