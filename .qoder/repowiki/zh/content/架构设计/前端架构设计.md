...
- 路由系统设计：基于 React Router v7 的 MemoryRouter 与 AppShell 主布局结合，提供统一导航与内容区，优化 WebView 环境下的性能和内存管理。
...
- 性能优化策略：并发数据加载、滚动行为控制、CSS 变量主题切换、移动端手势拦截、MemoryRouter 内存优化。
- 跨浏览器兼容性考虑：CSS 自定义属性、渐进增强样式、移动端滚动与触摸事件处理。

## 项目结构
前端采用"按功能域分层"的组织方式：
- 根入口与渲染：main.tsx 渲染根组件 App.tsx
- 应用主路由与布局：App.tsx 配置 MemoryRouter 与 Routes，包裹 AppShell 提供统一布局
...
## 核心组件
- App.tsx：应用启动与全局初始化，包含日志初始化、药物提醒服务启动与清理、移动端手势拦截（完全禁用横向滑动手势）。**更新**：路由系统从 BrowserRouter 改为 MemoryRouter，优化 WebView 环境下的性能和内存管理。
...
### 路由系统设计
- 根路由：**更新**：从 BrowserRouter 改为 MemoryRouter，适用于 WebView 环境，提供更好的内存管理和性能表现
...
AppShell->>Router : 加载应用
Router->>Routes : 解析路由
Routes->>Shell : 渲染 AppShell
Shell->>Shell : 初始化数据库与设置
Shell-->>Routes : 渲染完成
Routes->>Page : 渲染目标页面
Page-->>AppShell : 显示页面内容
```

图表来源
- [src/App.tsx:32-52](file://src/App.tsx#L32-L52)
...
图表来源
- [src/App.tsx:35-47](file://src/App.tsx#L35-L47)

章节来源
- [src/App.tsx:29-47](file://src/App.tsx#L29-L47)
...
- **更新**：MemoryRouter 优化：在 WebView 环境下使用 MemoryRouter 替代 BrowserRouter，显著降低内存占用和提升导航性能，特别适合 Tauri 桌面应用和混合应用场景
...
- [src/App.tsx:32](file://src/App.tsx#L32)
...
- **更新**：路由性能问题：如遇到 WebView 环境下路由卡顿或内存泄漏，确认已使用 MemoryRouter 替代 BrowserRouter，MemoryRouter 不依赖浏览器历史记录 API，更适合原生应用环境
...
## 结论
Assetly 前端以 React 19.1 为核心，结合 Zustand 实现 MVVM 中的 ViewModel 层，通过 AppShell 提供统一布局与响应式导航，并以并发数据加载与 CSS 变量主题切换保障性能与体验。配合 Tauri 的 SQLite 数据库与日志能力，形成从 UI 到数据的完整闭环。**更新**：路由系统优化采用 MemoryRouter 替代 BrowserRouter，在 WebView 环境下显著提升性能和内存管理效率。该架构在组件化、类型安全、移动端手势处理与跨浏览器兼容方面均有明确实践，适合在多端场景下持续演进。