...
- **药品类型管理**：支持7种预定义的药品类型（内服、外用、急救、注射、吸入、眼用、外用膏贴）
- **扩展单位系统**：预定义的中文药品计量单位（片、粒、支、瓶、盒、袋、包、条、贴、枚、滴、喷、克、毫克、毫升）
- **库存跟踪**：实时库存管理和数量调整
- **过期提醒**：智能过期预警和到期提醒
- **用药提醒**：可配置的用药频率和时间安排
- **分类管理**：自定义分类和标签系统
...
**更新** 药品类别已从15种简化为7种固定预设：

系统支持 7 种预定义的药品类型，每种类型都有对应的中文标签：
...
**章节来源**
- [src/utils/constants.ts:15-24](file://src/utils/constants.ts#L15-L24)
- [src/stores/useSettingsStore.ts:10-26](file://src/stores/useSettingsStore.ts#L10-L26)

### 扩展单位系统

**更新** 单位系统已从可配置改为固定中文单位，新增多种医疗常用单位：

系统支持 13 种预定义的中文药品计量单位：
...
**章节来源**
- [src/routes/MedicineForm.tsx:200-226](file://src/routes/MedicineForm.tsx#L200-L226)
