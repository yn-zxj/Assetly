...
## 更新摘要
**变更内容**
- EmojiPicker组件已从外部 emoji-picker-react 库迁移到内置实现，使用静态表情列表替代动态库依赖
- 设置界面已移除药品相关的配置选项，仅保留主题色和货币符号设置
- 新增表情选择器的静态表情列表实现和设置存储的简化
...
### 表情选择器 EmojiPicker
- **已更新**：组件已从外部 emoji-picker-react 库迁移到内置实现
- 功能要点
  - **新增**：使用静态表情列表 EMOJI_LIST 替代动态库依赖
  - **新增**：网格布局的表情面板，按类别分组展示 26 个不同类别的表情
  - 点击回调返回所选表情字符
- Props
  - onSelect: (emoji: string) => void
- 状态与逻辑
  - 内置 EMOJI_LIST 静态数组，包含 26 个表情类别
  - 支持 8 列网格布局，最大高度限制为 48 个单位
- 样式与主题
  - 固定 8 列网格，滚动容器限制高度
  - 悬停态过渡效果，圆角背景
- 无障碍与可用性
...
**章节来源**
- [EmojiPicker.tsx:1-3](file://src/components/shared/EmojiPicker.tsx#L1-L3)
- [EmojiPicker.tsx:5-26](file://src/components/shared/EmojiPicker.tsx#L5-L26)
...
- 组件间耦合
  ...
  - 业务页面（ItemForm、MedicineForm、Settings）作为组合容器，将多个通用组件编排为表单
- 外部依赖
  - 日期处理：dayjs（用于格式化与计算）
  - 图标：lucide-react
  - 样式：Tailwind + 主题变量
...
- [Settings.tsx:7](file://src/routes/Settings.tsx#L7)
...
- 组件级优化
  ...
  - **新增**：EmojiPicker：静态表情列表，减少外部依赖和包体积
- 主题与样式
...
- **新增**：表情选择器不可用
  ...
  - 检查 EMOJI_LIST 是否正确导入
  - 确认网格布局样式是否正常
- **新增**：设置界面主题色不生效
  - 检查 useSettingsStore 中的 setThemeColor 方法
  - 确认 CSS 变量 --color-primary 是否正确设置
  - 验证数据库中 settings 表的 theme_color 字段
...
## 结论
上述通用组件以简洁的 Props/回调模型与稳定的内部状态管理，实现了高复用与强组合能力。结合主题变量与统一的交互模式，可在多页面中快速搭建一致的用户体验。

**主要更新**：
- EmojiPicker 组件已完全重构为内置实现，移除了外部依赖，提升了包体积效率
- 设置界面简化为仅包含主题色和货币符号配置，去除了复杂的药品相关设置
- 新增了静态表情列表，提供了更可控的表情选择体验

建议在后续迭代中补充无障碍属性与键盘支持，并对长列表引入虚拟化以进一步提升性能。

## 附录：使用示例与最佳实践
- 在表单中组合使用
  - 物品表单：搜索栏用于筛选、日期选择器用于购买/保质期、自定义下拉用于分类/状态、表情选择器用于图标
  - 药品表单：日期选择器用于有效期/购买日期、时间选择器用于服药时间、自定义下拉用于类型/单位、确认对话框用于删除
  - **新增**：设置页面：表情选择器用于图标选择、确认对话框用于数据导入导出
- 最佳实践
  - 使用受控组件，确保父组件持有真实状态
  - 为关键交互提供可访问性属性（aria-*）
  ...
  - **新增**：表情选择器使用静态列表，避免外部依赖带来的兼容性问题
  - **新增**：设置界面使用本地存储，确保用户偏好持久化
...
- [Settings.tsx:140-190](file://src/routes/Settings.tsx#L140-L190)
- [Settings.tsx:274-283](file://src/routes/Settings.tsx#L274-L283)