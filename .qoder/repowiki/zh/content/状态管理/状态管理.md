...
- 设置域 Store（useSettingsStore）：负责主题色与货币符号等应用设置的读取与持久化。
...
DB-->>Store : 返回键值对
Store->>Store : 解析并设置 themeColor/currencySymbol/loaded
UI->>Store : setThemeColor(color)
Store->>DB : INSERT OR REPLACE settings(key='theme_color', value, updated_at)
DB-->>Store : 成功
Store->>Store : 更新 themeColor
Store->>UI : 触发重渲染DOM CSS 变量已更新
```

图表来源
- [useSettingsStore.ts:19-54](file://src/stores/useSettingsStore.ts#L19-L54)
- [Dashboard.tsx:15-16](file://src/routes/Dashboard.tsx#L15-L16)

章节来源
- [useSettingsStore.ts:5-55](file://src/stores/useSettingsStore.ts#L5-L55)
...
- 设置未生效
  - 检查数据库写入是否成功；确认 DOM CSS 变量赋值是否执行。
- 性能问题
  - 关注渲染次数与重渲染范围；对大列表启用虚拟滚动或分页；避免在渲染中做昂贵计算。

## 结论
Assetly 的状态管理以 Zustand 为核心，采用“按领域拆分”的 Store 设计，清晰划分职责、统一类型约束、通过服务层隔离副作用。组件通过订阅 Store 实现响应式更新，配合并行加载与简洁的刷新策略，在保证开发效率的同时兼顾性能与可维护性。遵循本文最佳实践与排错建议，可进一步提升系统的稳定性与扩展性。
...
- Store 使用清单
  ...
  - 应用设置：订阅 useSettingsStore，调用 loadSettings 与 setThemeColor/setCurrencySymbol
- 状态订阅与响应式更新
