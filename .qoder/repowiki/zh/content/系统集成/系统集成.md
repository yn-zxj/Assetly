...
## 更新摘要
**变更内容**
- 新增Android文件共享机制：FileProvider配置与JavaScript接口桥接
- 更新Android权限配置：增强文件共享相关权限声明
- 完善Settings.tsx中的Android文件共享实现流程
- 扩展文件路径范围：支持更广泛的文件操作权限
...
## 项目结构
项目采用前端（React + Vite）与后端（Rust/Tauri）混合架构，核心业务逻辑位于前端，通过 Tauri 插件访问系统能力（通知、日志、文件系统、SQL 数据库等）。Android 平台通过原生广播接收器实现系统级推送通道，并通过JavaScript接口桥接实现文件共享功能。
...
MAIN["MainActivity.kt<br/>JavaScript接口桥接"]
...
- [src-tauri/gen/android/app/src/main/java/com/assetly/home/MainActivity.kt:53-109](file://src-tauri/gen/android/app/src/main/java/com/assetly/home/MainActivity.kt#L53-L109)
...
- 文件系统集成：设置页支持移动端"分享导出"与桌面端浏览器下载；导入时进行 JSON 校验与批量写入；Android 通过FileProvider实现安全文件分享。
...
participant Main as "MainActivity.kt<br/>JavaScript接口桥接"
...
- **新增** Android 文件共享机制
  - 通过 JavaScript 接口桥接实现原生文件分享功能。
  - 使用 FileProvider 安全地分享临时文件，避免权限问题。
  - 支持缓存目录文件写入和URI授权。
...
participant JS as "MainActivity.kt<br/>JavaScript接口"
UI->>Exp : 导出 JSON
Exp-->>UI : 返回 JSON 字符串
UI->>JS : Android.shareTextAsFile(...)
JS->>JS : 写入缓存目录文件
JS->>JS : FileProvider获取URI
JS->>And : 打开系统分享面板
And-->>UI : 打开系统分享面板
```
...
- [src-tauri/gen/android/app/src/main/java/com/assetly/home/MainActivity.kt:76-109](file://src-tauri/gen/android/app/src/main/java/com/assetly/home/MainActivity.kt#L76-L109)
...
- 平台特性
  ...
  - **新增** JavaScript 接口桥接：通过 WebView.addJavascriptInterface 提供原生功能访问。
...
MAIN["MainActivity.kt"] --> JS["JavaScript接口"]
...
- 组件耦合
  ...
  - **新增** MainActivity.kt 通过 JavaScript 接口桥接提供原生文件分享功能。
- 外部依赖
  - Tauri 插件：notification、log、fs、sql。
  - Android 原生：NotificationChannel、BroadcastReceiver、FileProvider、WebView JavaScript 接口。
...
Android --> Main["MainActivity.kt"]
Main --> Provider["FileProvider"]
Main --> JS["JavaScript接口"]
```
...
- **新增** Android 文件共享性能
  - 缓存目录文件写入避免外部存储权限问题，提升分享成功率。
  - JavaScript 接口调用在主线程执行，避免WebView线程阻塞。
...
- **新增** Android 文件分享失败
  - 检查 JavaScript 接口是否正确注册；确认 FileProvider 配置是否正确；验证缓存目录权限。
  - 查看 Android 日志输出的错误信息，确认文件写入和URI授权是否成功。
...
- [src-tauri/gen/android/app/src/main/java/com/assetly/home/MainActivity.kt:104-107](file://src-tauri/gen/android/app/src/main/java/com/assetly/home/MainActivity.kt#L104-L107)

## 结论
Assetly 的系统集成功能以 Tauri 插件为核心，结合前端定时任务与 Android 原生广播接收器，实现了稳定的通知与文件操作体验。日志系统提供了结构化与可视化的可观测性。通过明确的权限配置与作用域约束，兼顾了安全性与可用性。**新增的 Android 文件共享机制**通过 FileProvider 和 JavaScript 接口桥接，提供了更加可靠和安全的文件分享体验。建议在生产环境中进一步优化通知频率、日志持久化与导入事务处理，并持续监控 Android 通知渠道与权限变化。
...
- 最佳实践
  ...
  - **新增** Android 文件分享：使用缓存目录避免权限问题；通过 FileProvider安全分享文件；提供多级回退机制。
- 参考路径
  ...
  - **新增** Android JavaScript接口桥接：[src-tauri/gen/android/app/src/main/java/com/assetly/home/MainActivity.kt:53-109](file://src-tauri/gen/android/app/src/main/java/com/assetly/home/MainActivity.kt#L53-L109)
  - **新增** Settings.tsx Android文件分享实现：[src/routes/Settings.tsx:29-71](file://src/routes/Settings.tsx#L29-L71)